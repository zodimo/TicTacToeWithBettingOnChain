...
import { Game, GameIsTiePayout, Payout } from "../../app/game.js";
import { ClaimTieCommand, GameStateFactory } from "../../app/game-data.js";
...
export const sendClaimTieCommandToScriptTransaction: (
  playerWallet: Wallet,
  ...
  const command = new ClaimTieCommand(gameStateFromScriptData);
  ...
  const gamePayoutAsTxOutParameters: (payout: Payout, wallet: Wallet) => TxOutParameter = (payout, wallet) => {
    ...
    assert.equal(winningWalletPubKeyHash == payout.pubKeyHash.toHexString(), true, "Expects pub key hash to match!");
  ...
  const payouts = (gamePayout as GameIsTiePayout).payouts;
  const wallet1 = payOutToWallet(payouts[0], playerWallets);
  const wallet2 = payOutToWallet(payouts[1], playerWallets);
  ...
  const transactionBuildOptions = new TransactionBuildOptions()
    ...
    .withTxOut(gamePayoutAsTxOutParameters(payouts[0], wallet1))
    .withTxOut(gamePayoutAsTxOutParameters(payouts[1], wallet2))
  ...
  const wallet1TxUtxoId = new UtxoId(txHash, 0);
  const wallet2TxUtxoId = new UtxoId(txHash, 1);

  //wait for one of the utxos to arrive
  cardanoCli.waitForUtxoAtPaymentAddress(wallet1.paymentAddr, wallet1TxUtxoId, 600);
  return [wallet1TxUtxoId, wallet2TxUtxoId];
};
